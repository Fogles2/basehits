...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['ajax_reply'])) {
    ...
    $modResult = $moderator->moderateMessage($message, $_SESSION['user_id']);

    if($modResult['success'] && $moderator->shouldReject($modResult)) {
        echo json_encode(['success' => false, 'error' => 'Message blocked: ' . $modResult['reason']]);
        exit;
    }
